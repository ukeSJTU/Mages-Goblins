...
TASKS = [
    ("1_task1", ["algorithms.cpp", "engine.cpp", "field.cpp", "units.cpp", "main.cpp"]),
    ("2_task2", ["algorithms.cpp", "engine.cpp", "field.cpp", "units.cpp", "main.cpp"]),
    ("3_task3", ["algorithms.cpp", "engine.cpp", "field.cpp", "units.cpp", "main.cpp"]),
    ("4_task4", ["algorithms.cpp", "engine.cpp", "field.cpp", "units.cpp", "main.cpp"]),
]
...
# 定义不同情况下的颜色
COLOR_CORRECT = "\033[92m"  # 绿色
COLOR_RUNTIME_ERROR = "\033[91m"  # 红色
COLOR_WRONG_ANSWER = "\033[93m"  # 黄色
...
def get_cs1604_path(cs1604_txt):
    ...
    try:
        ...
        print(
            ...
            cs1604_txt,
            "whose contents is the absolute path " "to the compiled StanfordCppLib.",
            sep="\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def parse_args():
    ...
    parser.add_argument(
        "-T",
        "--task",
        type=TASK_NAME_TO_I.__getitem__,
        action="append",
        choices=TASK_NAMES,
    ...
    # verbose is a flag that controls whether to show expected and actual output diff when wrong
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="show expected and actual output diff when wrong",
    )
    ...
    if os.path.exists(cs1604_txt):
        ...
        args.cs1604_ldargs = [f'-L{os.path.join(cs1604_path, "lib")}', "-lCS1604"]
    else:
        args.cs1604_cxxargs = []
        args.cs1604_ldargs = []
...
def judge(args, task, case, workdir, exe_path):
    ...
    with fin:
        ...
        try:
            subprocess.run(
                [exe_path], check=True, timeout=args.timeout, stdin=fin, stdout=fdout
            )
        except subprocess.TimeoutExpired:
            raise Wrong("Timed out")
        except subprocess.CalledProcessError as e:
            raise Wrong(
                f"{COLOR_RUNTIME_ERROR}Runtime error with return code "
                f"{e.returncode} ({e.returncode:#x}){COLOR_RESET}"
            )
    ...
    try:
        with open(out_path) as fout:
            with open(std_path) as fstd:
                ...
                if any(l[0] != " " for l in diff):
                    extra_msg = (
                        ("[DIFF] " + l for l in diff) if args.verbose else ()
                    )  # 根据verbose选项控制
                    raise Wrong(
                        f"{COLOR_WRONG_ANSWER}Incorrect output{COLOR_RESET}", extra_msg
                    )
    except Wrong as e:
        if e.score == 0:
            try:
        ...
        try:
            ...
            for case in args.case:
                ...
                try:
                    ...
                    print(tc, f"{COLOR_CORRECT}Correct{COLOR_RESET}")
        except Wrong as e:
...
def compile_and_link(args, task, workdir):
    ...
    src_paths = [os.path.join(args.thisdir, "source", task_name, src) for src in srcs]
